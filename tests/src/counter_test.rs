...
mod test {
    ...
    use solana_sdk::{msg, system_program};
    use solana_sdk::transaction::Transaction;
    use crate::{DEV_NET_URL, MY_WALLET_PRIVATE_KEY};

    #[tokio::test]
    async fn test() {
        let program_id = Pubkey::from_str("7GwjBcoAiS9czaYajf4habevN5MSmcgRP8cTYU8BZ4nh").unwrap();
        ...
        println!("计数器公钥 {}", counter_keypair.pubkey());
        // 3.1 创建计数器的指令
        ...
        transaction.sign(&[&wallet, &counter_keypair], latest_blockhash);
